...
export function DiscoverClient({ userId, swipedMovieIds: initialSwiped, totalSwipes: initialTotal, personalityLabel: initialLabel }: Props) {
  ...
  const handleSwipe = useCallback((action: "like" | "pass" | "watchlist") => {
    const movie = movies[currentIndex];
    if (!movie) return;
  ...
  const submitSwipe = useCallback(async (movieId: string, action: "like" | "pass" | "watchlist", reasons: string[]) => {
    try {
  ...
  if (loading && movies.length === 0) {
    return (
      <div style={{ minHeight: "100svh", display: "flex", alignItems: "center", justifyContent: "center", background: "#0e0e0f" }}>
        <p style={{ fontSize: "13px", color: "rgba(232,227,216,0.4)" }}>映画を読み込んでいます…</p>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100svh", background: "#0e0e0f", display: "flex", flexDirection: "column", maxWidth: "430px", margin: "0 auto" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "0 16px" }}>
        {pendingAction ? (
          ...
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            <p style={{ fontSize: "16px", color: "rgba(232,227,216,0.6)", marginBottom: "8px" }}>ひとまず全部見ました</p>
            <p style={{ fontSize: "13px", color: "rgba(232,227,216,0.35)" }}>新しい映画が追加されるとまた表示されます</p>
          </div>
        ) : (
      ...
      {!pendingAction && visibleCards.length > 0 && (
        <div style={{ padding: "12px 20px 32px", display: "flex", justifyContent: "space-between" }}>
          <span style={{ fontSize: "11px", color: "rgba(232,227,216,0.2)" }}>← パス</span>
          <span style={{ fontSize: "11px", color: "rgba(232,227,216,0.2)" }}>↑ 後で</span>
          <span style={{ fontSize: "11px", color: "rgba(232,227,216,0.2)" }}>興味あり →</span>
        </div>
      )}
      ...
      {milestone100 && (
        <>
          ...
          <div style={{
            ...
            <p style={{ fontSize: "16px", fontWeight: 600, color: "#E8C97A", marginBottom: "8px" }}>映画人格が生成されました</p>
            {personalityLabel && (
